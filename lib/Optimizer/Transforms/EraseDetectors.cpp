...
public:
  ...
  LogicalResult matchAndRewrite(quake::DetectorOp detector,
    ...
    // First determine if the arguments originate from cc.stdvec_init operations.
    if (auto stdvecInit = dyn_cast<cudaq::cc::StdvecInitOp>(detector.getOperands()[0].getDefiningOp())) {
...
class EraseDetectorsPass : public cudaq::opt::impl::EraseDetectorsBase<EraseDetectorsPass> {
public:
